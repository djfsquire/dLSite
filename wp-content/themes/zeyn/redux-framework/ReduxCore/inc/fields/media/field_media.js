...
function redux_add_logo(event, selector) {
	...
	frame.on( 'select', function() {
		...
		selector.find('.upload-logo').val(attachment.attributes.url);
		selector.find('.upload-logo-id').val(attachment.attributes.id);
/*
		selector.find('.upload-height').val(attachment.attributes.height);
		selector.find('.upload-width').val(attachment.attributes.width);
*/
		redux_change( jQuery(selector).find( '.upload-logo-id' ) );
		var thumbSrc = attachment.attributes.url;
		if (typeof attachment.attributes.sizes !== 'undefined' && typeof attachment.attributes.sizes.thumbnail !== 'undefined') {
			thumbSrc = attachment.attributes.sizes.thumbnail.url;
		} else if ( typeof attachment.attributes.sizes !== 'undefined' ) {
		...
		selector.find('.upload-logo-thumbnail').val(thumbSrc);
		if ( !selector.find('.upload').hasClass('noPreview') ) {
			selector.find('.logoscreenshot').empty().hide().append('<img class="redux-option-image" src="' + thumbSrc + '">').slideDown('fast');
		}
		//selector.find('.media_upload_button').unbind();
		selector.find('.remove-logo').removeClass('hide');//show "Remove" button
		selector.find('.redux-background-properties').slideDown();
...
function redux_remove_logo(selector) {

	// This shouldn't have been run...
	if (!selector.find('.remove-image').addClass('hide')) {
		return;
	}
	selector.find('.remove-logo').addClass('hide');//hide "Remove" button
	selector.find('.upload-logo').val('');
	selector.find('.upload-logo-id').val('');
//	selector.find('.upload-height').val('');
//	selector.find('.upload-width').val('');
	redux_change( jQuery(selector).find( '.upload-logo-id' ) );
	selector.find('.redux-background-properties').hide();
	var screenshot = selector.find('.logoscreenshot');
	
	// Hide the screenshot
	screenshot.slideUp();

	selector.find('.remove-logo').unbind();
	// We don't display the upload button if .upload-notice is present
	// This means the user doesn't have the WordPress 3.5 Media Library Support
	if ( jQuery('.section-upload .upload-notice').length > 0 ) {
		jQuery('.logo_upload_button').remove();
	}
...
(function($){
	...
	*/
    $.redux.media = function(){
		...
		$('.remove-logo').unbind('click').on('click', function() {
			redux_remove_logo( $(this).parents('fieldset.redux-field:first') );
		});

		$('.logo_upload_button').unbind().on('click', function( event ) {
			redux_add_logo( event, $(this).parents('fieldset.redux-field:first') );
		});
