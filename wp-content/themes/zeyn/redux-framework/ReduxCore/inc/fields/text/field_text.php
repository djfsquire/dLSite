<?php
class DethemeReduxFramework_text extends DethemeReduxFramework {
    ...
    function render() {
    	...
    	if ( isset( $this->field['options'] ) && !empty( $this->field['options'] ) ) {
			...
			foreach($this->field['options'] as $k => $v){
                ...
                echo '<input type="text" id="' . $this->field['id'] . '-text-'.$k.'" name="' . $this->args['opt_name'] . '[' . $this->field['id'] . ']['.$k.']" ' . $placeholder . 'value="' . ((in_array($k,array_keys($this->value)))?esc_attr($this->value[$k]) :"").'" class="regular-text ' . $this->field['class'] . '" /><br />';
